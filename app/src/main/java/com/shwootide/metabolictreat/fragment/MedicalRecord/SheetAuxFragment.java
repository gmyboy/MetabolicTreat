 ...
 */
public class SheetAuxFragment extends BaseFragment {

    @Bind(R.id.lv_diagnosis1)
    GridView gvDiagnosis2;
    @Bind(R.id.slhlv_dialog_past1)
    ExpandableListView slhlvDialogPast2;
    @Bind(R.id.tv_title_sheet)
    TextView tvTitleSheet;

    private SheetAdapter adapter_r2;
    private ExpandSheetAdapter adapter_l2;

    public static SheetAuxFragment newInstance(String illnessid, int mPosition, int mSequenceNumber) {
        Bundle args = new Bundle();
        args.putString("IllnessID", illnessid);
        args.putInt("Position", mPosition);
        args.putInt("SequenceNumber", mSequenceNumber);
        SheetAuxFragment fragment = new SheetAuxFragment();
    ...
    public void onEventMainThread(MessageEvent event) {
        ...
        if (event.what.equals("GetAllStandardInfo56") && isVisible) {
            if (event.getCode().equals("200") && !CommonUtil.isEmpty(event.getObjects())) {
                adapter_r2 = new SheetAdapter(getActivity());
                adapter_l2 = new ExpandSheetAdapter(getActivity(), 1, event.getObjects());
                slhlvDialogPast2.setAdapter(adapter_l2);
                gvDiagnosis2.setAdapter(adapter_r2);
                mHasLoadedOnce = true;
                slhlvDialogPast2.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
                    @Override
                    public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                        adapter_l2.setClicked(groupPosition, childPosition);
                        adapter_r2.upate(CommonUtil.fromSheet(adapter_l2.getChild(groupPosition, childPosition)));
                        return false;
                    }
                });
            }
    ...
    private void initDate(MessageEvent event) {
        ...
        adapter_r2 = new SheetAdapter(getActivity(), getDatas(event.getObjects()));
        gvDiagnosis2.setAdapter(adapter_r2);
    }
    ...
    public void certainSubmit() {
        ...
        if (adapter_r2 != null && adapter_r2.getCount() != 0) {
            Sheet_Submit submit;
            for (int i = 0; i < adapter_r2.getCount(); i++) {
                submit = adapter_r2.getItem(i);
